...
export class LandingSlide4 extends React.Component < {}, {} > {
  submitForm = (event : any) => {
    ...
    emailjs.sendForm('service_jlqh91p', 'template_rjyxj03', ".emailForm", "user_Es5RXdkMrmbS4UFtZfAhb").then(function(response) {
      console.log('SUCCESS!', response.status, response.text);
    }, function(error) {
      console.log('FAILED...', error);
    });
    //clear email field
    let EmailAddressField: HTMLInputElement = (document.getElementById("EmailAddressField"));
    EmailAddressField.value = "";

  }
  render() {
    return (<div className="Landing4">
      <div className="OmappText">
        <h1 className="whiteText TitleText" style={{
            marginBottom: 10
          }}>Omapp</h1>
      ...
      <div className="emailInputField" id="emailSection">
        <form onSubmit={this.submitForm} className="emailForm ">
          <InputGroup className="mb-3">
            <FormControl className="form-control" id="EmailAddressField" type="email" name="email" placeholder="email*" aria-describedby="basic-addon2" /*style={{ backgroundColor: "black", color: "#0000FF", border: "2px solid rgb(0,123,255)"}}*/
            />
